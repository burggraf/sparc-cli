...
- Next, observe schema/database ownership and add local security-definer/view effects; then define a reviewed expected destination profile rather than allowing source state. Do not treat the current guard as complete; wire it to a restore gate only when a real restore pipeline exists.
