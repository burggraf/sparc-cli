...
`ObserveCatalog` now reports selected relation owner role names as facts, but this first guard does **not** compare them to an expected profile. The guard also does not evaluate named roles such as Supabase `anon`, `authenticated`, or `service_role`; memberships, grantors/options, policy behavior, security-definer bodies, application collisions, target identity, or changes after observation. A pass means only “no observed `PUBLIC SELECT` in this narrow scope,” not “safe target,” “permission-equivalent,” or restore-ready. No restore command calls this guard.
...
The test directly grants `SELECT` on one public table to `PUBLIC`, observes only the object-level grant (no default or column grants), and confirms `CheckTargetSecurityV1` refuses. A separate ordinary login role reads the synthetic canary, reproducing the exposure. A second case transfers a synthetic relation to a `NOLOGIN` role and confirms `ObserveCatalog` reports the role name rather than a local OID; the guard does not compare that observation to an expected owner profile. This is read-only preflight evidence after fixture seeding; it does **not** prove refusal before an actual restore mutation because no product restore path calls the guard.
