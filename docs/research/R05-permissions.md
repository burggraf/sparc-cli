...
`ObserveCatalog` reports selected relation owner names and bounded relation/column ACL facts, explicit global and selected-schema `pg_default_acl` entries, cluster-wide role attributes, and membership edges with PG17 grantor/admin/inherit/set options. It translates catalog OIDs to role names and does not query `pg_authid`, password hashes, or role settings. This first guard does **not** compare owner, ACL, default, role, or membership facts to an expected profile. It also does not evaluate grants to named Supabase roles such as `anon`, `authenticated`, or `service_role`; membership consequences, policy behavior, security-definer bodies, application collisions, target identity, or changes after observation. A pass means only “no observed `PUBLIC SELECT` in this narrow scope,” not “safe target,” “permission-equivalent,” or restore-ready. No restore command calls this guard.
...
**Verification 2026-09-23:** Full default and localdemo-tagged test suites, PostgreSQL 17.9 integration and race suites, default/tagged/integration vet, CLI and demo builds, Windows AMD64/Darwin arm64 integration-test cross-compiles, end-to-end local rehearsal plus offline verify, formatting, and `git diff --check` passed on macOS arm64. A new isolated PostgreSQL 17.9 case also passes:
...
- Next, observe RLS policy roles/effects and schema/database ownership; add local owner/BYPASSRLS, enabled/forced-RLS and missing-policy, and security-definer/view cases. Then define a reviewed expected destination profile rather than allowing source state. Do not treat the current guard as complete; wire it to a restore gate only when a real restore pipeline exists.
